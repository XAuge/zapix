...
var jsonRpc = (function($) {
  ...
  function removeEmpty(obj) {
    $.each(obj, function(key, value) {
      if (value && typeof value === 'object') removeEmpty(obj[key]);
      else if (value == null || value == "") delete obj[key];
      if (typeof value === 'object' && Object.keys(obj[key]).length == 0) delete obj[key];
    });
  };
    ...
    call: function(method, params, onSuccess) {
      ...
      $.ajax({
        ...
        complete: function(xhr,status) {
          ...
          $("#response").replaceWith('<code id="response">' + JSON.stringify(data) + '</code>');
          $("#response").rainbowJSON();
          if (FormatRequest == true) {
            $("#request").replaceWith('<code id="request">' + request + '</code>');
            $("#request").rainbowJSON();
          }
        }
      });
